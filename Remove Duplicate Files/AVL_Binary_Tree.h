...
class AVL_Binary_Tree{
		...
		//Node structure that make up the tree.
	struct Node{
		...
		// Used to delete the full tree from leafs up.
	...
	private: void Insert_Not_Root(T &InsertObject, long long key, std::vector<T> &Duplicate_Files, Node* Argument = NULL, int height = 1){
		...
		if (Argument->key == key){
			...
			Argument->Data.push_back(InsertObject);
	...
	private: int Find_Height(Node * Argument = NULL, int height_arg = 0){
		...
		// To fix the height after rotations.
